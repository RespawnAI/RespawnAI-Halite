...
game = hlt.Game("Respawn AI")
logging.info("Respawn Live Long and Prosper")
...
while True:
    ...
    for ship in game_map.get_me().all_ships():
        ...
        closest_enemy_ships = [entities_by_distance[distance][0] for distance in entities_by_distance if
                isinstance(entities_by_distance[distance][0], hlt.entity.Ship) and entities_by_distance[distance][0] not
                in team_ship]
        ...
        if len(closest_empty_planets) > 0:
            ...
            if ship.can_dock(target_planet):
                ...
                moveFull(target_planet)
        # FIND SHIP TO ATTACK!
        elif len(closest_enemy_ships) > 0:
            target_ship = closest_enemy_ships[0]
            moveFull(target_ship)
